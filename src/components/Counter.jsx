...
useEffect(() => {
  ...
  const handleKeyDown = (e) => {
    ...
    // Fixed: Use e.code instead of e.key for Shift combos
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 justify-center mb-2">
        <button onClick={increment} disabled={count >= MAX} className={`bg-teal-500 hover:bg-teal-400 text-white px-3 py-2 rounded font-bold text-sm transition ${count >= MAX? disabledClass : ''}`}>+1</button>
        <button onClick={decrement} disabled={count <= MIN} className={`bg-rose-500 hover:bg-rose-400 text-white px-3 py-2 rounded font-bold text-sm transition ${count <= MIN? disabledClass : ''}`}>-1</button>
        <button onClick={reset} className="bg-neutral-600 hover:bg-neutral-500 text-white px-3 py-2 rounded font-bold text-sm transition">Reset</button>
        <button onClick={addFive} disabled={count >= MAX} className={`bg-cyan-500 hover:bg-cyan-400 text-white px-3 py-2 rounded font-bold text-sm transition ${count >= MAX? disabledClass : ''}`}>+5</button>
        <button onClick={subFive} disabled={count <= MIN} className={`bg-pink-500 hover:bg-pink-400 text-white px-3 py-2 rounded font-bold text-sm transition ${count <= MIN? disabledClass : ''}`}>-5</button>
        <button onClick={double} disabled={count >= MAX || count * 2 > MAX} className={`bg-amber-500 hover:bg-amber-400 text-white px-3 py-2 rounded font-bold text-sm transition ${count >= MAX || count * 2 > MAX? disabledClass : ''}`}>x2</button>
      </div>
      
      <button onClick={undo} disabled={history.length === 0} className={`w-full bg-indigo-500 hover:bg-indigo-400 text-white px-3 py-2 rounded font-bold text-sm transition mb-3 ${history.length === 0? disabledClass : ''}`}>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[0, 1, 2].map(i => (
            <div key={i} className="flex flex-col gap-1">
              <button 
                onClick={() => saveSlot(i)} 
                className="bg-emerald-600 hover:bg-emerald-500 text-white px-2 py-1 rounded text-xs font-bold transition"
              >
                Save {i + 1}
              </button>
              <button 
                onClick={() => loadSlot(i)} 
                disabled={!slots[i]}
                className={`bg-sky-600 hover:bg-sky-500 text-white px-2 py-1 rounded text-xs font-bold transition ${!slots[i]? disabledClass : ''}`}
              >
